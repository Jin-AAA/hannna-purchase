...
function App() {
  ...
  return <main className="portal-shell">
    <header><Brand /><div className="header-account-actions"><button onClick={() => setChangingPassword(true)}><KeyRound size={16} />更改密碼</button><button onClick={() => signOut(auth)}><LogOut size={16} />登出</button></div>{changingPassword && <PasswordPanel view={view} user={user} onClose={() => setChangingPassword(false)} />}</header>
